...
class TestBreakCommand(unittest.TestCase):
    ...
    def test_parse_break_cmd(self):
        ...
        fn, fi, li, cond = Mbreak.parse_break_cmd(self.cmd, [])
        self.assertEqual((None, True, True), 
                         (fn, fi.endswith('test-break.py'), li > 1))

        fn, fi, li, cond = Mbreak.parse_break_cmd(self.cmd, ['11-1'])
        self.assertEqual((None, True, 10), 
                         (fn, fi.endswith('test-break.py'), li))

        fn, fi, li, cond = Mbreak.parse_break_cmd(self.cmd, [__file__ + ':10'])
        self.assertEqual((None, True, 10), 
                         (fn, isinstance(fi, types.StringType), li))

        def foo():
            return 'bar'
        fn, fi, li, cond = Mbreak.parse_break_cmd(self.cmd, ['foo'])
        self.assertEqual((foo, True, True), 
                         (fn, fi.endswith('test-break.py'), li > 1))

        fn, fi, li, cond = Mbreak.parse_break_cmd(self.cmd, ['food'])
        self.assertEqual((None, None, None, None), (fn, fi, li, cond))

        fn, fi, li, cond = Mbreak.parse_break_cmd(self.cmd, ['os.path'])
        self.assertEqual((None, True, None), 
                         (fn, isinstance(fi, types.StringType), li))

        fn, fi, li, cond = Mbreak.parse_break_cmd(self.cmd, ['os.path', '5+1'])
        self.assertEqual((None, True, 6), 
                         (fn, isinstance(fi, types.StringType), li))

        fn, fi, li, cond = Mbreak.parse_break_cmd(self.cmd, ['os.path.join'])
        self.assertEqual((os.path.join, True, True), 
                         (fn, isinstance(fi, types.StringType), li > 1))

        fn, fi, li, cond = Mbreak.parse_break_cmd(self.cmd, ['if', 'True'])
        self.assertEqual((None, True, True), 
                         (fn, fi.endswith('test-break.py'), li > 1))
